...
from .models import (
    ...
    Publication, Activity, Project, ProjectMember,
    StudentProfile,
...
class ActivityFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        now = timezone.now()
        qs  = queryset.annotate(last_active=Max('audit_logs__created_at'))
        if self.value() == 'active':
            return qs.filter(last_active__gte=now - timedelta(days=30))
        if self.value() == 'idle':
            return qs.filter(
                last_active__gte=now - timedelta(days=180),
                last_active__lt=now  - timedelta(days=30),
            )
        if self.value() == 'inactive':
            return qs.filter(last_active__lt=now - timedelta(days=180))
        if self.value() == 'never':
            return qs.filter(last_active__isnull=True)
        return queryset
...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = (
        ...
        'profile_status', 'last_active_display', 'date_joined',
    )
    list_filter = ('user_type', 'approval_status', 'is_staff', ActivityFilter)
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if request.user.user_type != 'admin':
            fieldsets = [fs for fs in fieldsets if fs[0] != 'EDI Profile']
    ...
    @admin.action(description='🔐 Reset 2FA for selected users (deletes TOTP + backup codes)')
    def reset_2fa(self, request, queryset):
        from django_otp.plugins.otp_totp.models import TOTPDevice
        from django_otp.plugins.otp_static.models import StaticDevice

        skipped = reset_count = 0
            ...
            if totp_deleted or static_deleted:
                ...
                StudentNotification.objects.create(
                    ...
                    message='Your two-factor authentication has been reset by an administrator. '
                            'You will be prompted to set it up again on next login.'
                )
                log_action(
                    request, 'other', target=user,
                    summary=f'{request.user.get_full_name()} reset 2FA for {user.get_full_name()} ({user.email})'
                )

        if reset_count:
            self.message_user(
                request,
                f'2FA reset for {reset_count} user(s). They will be prompted to set it up again on next login.',
                messages.SUCCESS,
            )
        if skipped:
            self.message_user(
                request,
                f'{skipped} admin account(s) skipped — superuser required to reset admin 2FA.',
                messages.WARNING,
            )
        if not reset_count and not skipped:
            self.message_user(
                request,
                'No 2FA devices found for the selected users.',
                messages.INFO,
            )
    ...
    def reject_users(self, request, queryset):
        from config.models import Project, Funding, Publication, Activity, Education, Recognition
        rejected = 0
        for user in queryset:
            try:
                researcher = user.researcherprofile
                # log counts before deletion for audit trail
                pub_count  = Publication.all_objects.filter(researcher=researcher).count()
                act_count  = Activity.all_objects.filter(researcher=researcher).count()
                proj_count = Project.all_objects.filter(researcher=researcher).count()

                log_action(
                    request, 'user_rejected', target=user,
                    summary=f'{request.user.get_full_name()} rejected {user.get_full_name()} ({user.email})',
                    details={
                        'publications_deleted': pub_count,
                        'activities_deleted':   act_count,
                        'projects_deleted':     proj_count,
                    }
                )

                Project.all_objects.filter(researcher=researcher).delete()
                Funding.all_objects.filter(researcher=researcher).delete()
                Publication.all_objects.filter(researcher=researcher).delete()
                Activity.all_objects.filter(researcher=researcher).delete()
                ...
                log_action(
                    request, 'user_rejected', target=user,
                    summary=f'{request.user.get_full_name()} rejected {user.get_full_name()} ({user.email})',
                    details={'note': 'No researcher profile found'}
    )
        ...
        self.message_user(
            request,
            f'{rejected} user(s) rejected and all research data deleted.',
            messages.WARNING
        )
    ...
    @admin.action(description='🔑 Set temporary password for selected users')
    def reset_to_temp_password(self, request, queryset):
        import json as _json
        from config.views import log_action
        ...
        for user in queryset:
            ...
            log_action(
                ...
                target=user,
                summary=f'{request.user.get_full_name()} reset temp password for {user.get_full_name()} ({user.email})'
            )

        if users_data:
            import json

            payload = json.dumps(users_data, ensure_ascii=True)
            self.message_user(
                request,
                mark_safe(f"""
                    {len(users_data)} temporary password(s) generated.
                    <script>
                    document.addEventListener("DOMContentLoaded", function() {{
                        showTempPasswordModal({payload});
                    }});
                    </script>
                """),
                messages.WARNING,
    ...
    @admin.action(description='⏸ Deactivate selected users (preserves data)')
    def deactivate_users(self, request, queryset):
        skipped = deactivated = 0
        for user in queryset:
            if user.user_type == 'admin':
                skipped += 1
                continue
            ...
            user.save()
            log_action(
                request, 'user_deactivated', target=user,
                summary=f'{request.user.get_full_name()} deactivated {user.get_full_name()} ({user.email})'
            )
            deactivated += 1
        self.message_user(
            request,
            f'{deactivated} user(s) deactivated. Data preserved. {skipped} admin(s) skipped.',
            messages.WARNING,
        )
    ...
    def reactivate_users(self, request, queryset):
        reactivated = 0
        for user in queryset.filter(approval_status='inactive'):
            user.approval_status = 'approved'
            user.is_active = True
            user.save()
            log_action(
                request, 'user_approved', target=user,
                summary=f'{request.user.get_full_name()} reactivated {user.get_full_name()} ({user.email})'
            )
            reactivated += 1
        self.message_user(
            request,
            f'{reactivated} user(s) reactivated.',
            messages.SUCCESS,
        )

    @admin.action(description='🗑 Permanently delete inactive users (90+ days)')
    def purge_long_inactive(self, request, queryset):
        cutoff   = timezone.now() - timedelta(days=90)
        to_purge = queryset.filter(
            approval_status='inactive',
            user_type__in=['student', 'researcher'],
        ).annotate(
            last_active=Max('audit_logs__created_at')
        ).filter(last_active__lt=cutoff)

        count = to_purge.count()
        if count == 0:
            self.message_user(request, 'No eligible users to purge.', messages.INFO)
            return

        for user in to_purge:
            log_action(
                request, 'user_deleted', target=user,
                summary=f'{request.user.get_full_name()} permanently deleted {user.get_full_name()} ({user.email})'
            )
            user.delete()

        self.message_user(
            request,
            f'{count} inactive user(s) permanently deleted.',
            messages.WARNING,
        )
    ...
    def approval_status_badge(self, obj):
        colors = {
            ...
            'inactive': ('#64748b', '#f1f5f9'),  # ← add this line
        }
    ...
    def last_active_display(self, obj):
        ...
        if days > 180:
            ...
            color = '#166534'
        return format_html(
            '<span style="color:{}">{} days ago</span>', color, days
        )
...
class LinkStatusFilter(SimpleListFilter):
    title          = 'Link Status'
    ...
    def queryset(self, request, queryset):
        if self.value() == 'linked':
            return queryset.filter(linked_student__isnull=False)
        if self.value() == 'unlinked':
            return queryset.filter(linked_student__isnull=True)
        return queryset
...
class StudentProfileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('EDI Profile', {
            'classes': ('collapse',),
            'fields': (
                'gender', 'residency_status', 'indigenous_identity',
                'race_ethnicity', 'edi_profile_completed',
            ),
        }),
    )
...
class SupervisionRecordAdmin(admin.ModelAdmin):
    list_display  = (
        'student_name', 'get_researcher', 'degree_type',
        'status', 'institution', 'linked_status', 'start_date',
    )
    list_filter   = ('status', 'degree_type', LinkStatusFilter)
    search_fields = (
        'student_name',
        'linked_student__user__email',
        'researcher__user__first_name',
        'researcher__user__last_name',
        'institution',
    )
    ordering      = ('-start_date',)
    list_per_page = 30
    ...
    fieldsets = (
        ('Record', {
            'fields': (
                'researcher', 'student_name', 'degree_type',
                'supervision_role', 'status', 'institution', 'department',
            ),
        }),
        ('Dates', {
            'fields': (
                'start_date', 'end_date', 'degree_start_date',
                'degree_end_date', 'expected_date',
            ),
        }),
        ('Linking', {
            'fields': ('linked_student', 'manually_overridden'),
            'description': (
                'Use linked_student to manually link a RIMS student account. '
                'If multiple students share the same name, search here by email or ID.'
            ),
        }),
        ('Other', {
            'classes': ('collapse',),
            'fields': (
                'thesis_title', 'present_position', 'present_org',
                'residency_status', 'external_id', 'created_at', 'updated_at',
            ),
        }),
    )

    def get_researcher(self, obj):
        return obj.researcher.user.get_full_name()
    get_researcher.short_description = 'Researcher'
    get_researcher.admin_order_field = 'researcher__user__last_name'

    def linked_status(self, obj):
        if obj.linked_student:
            return format_html(
                '<span style="padding:2px 8px;border-radius:4px;font-size:11px;'
                'font-weight:700;background:#dcfce7;color:#166534;">Linked</span>'
            )
        return format_html(
            '<span style="padding:2px 8px;border-radius:4px;font-size:11px;'
            'font-weight:700;background:#fee2e2;color:#991b1b;">Unlinked</span>'
        )
    linked_status.short_description = 'Link Status'
    linked_status.admin_order_field = 'linked_student'

    def get_queryset(self, request):
        qs = super().get_queryset(request).select_related(
            'researcher__user', 'linked_student__user'
        )
...
class PublicationInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).filter(is_active=True, is_deleted=False)
...
class ProjectInline(admin.TabularInline):
    ...
    fields = (
        'title', 'role', 'status',
        'funding_organization', 'funding_type',
        'total_funding', 'funding_received', 'funding_kept_by_unb',
        'start_date', 'end_date', 'manually_overridden',
    )
...
class FundingAdmin(admin.ModelAdmin):
    ...
    def get_total(self, obj):
        return f'${float(obj.amount or 0):,.0f}'
    get_total.short_description = 'Amount'

    def get_year_range(self, obj):
        start = str(obj.start_date)[:4] if obj.start_date else '?'
        end   = str(obj.end_date)[:4]   if obj.end_date   else '?'
        return f'{start} → {end}' if start != end else start
    get_year_range.short_description = 'Period'
...
class ProjectAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'funding_organization', 'total_funding',
        'start_date', 'end_date', 'manually_overridden', 
    )
...
admin.site.__class__ = RIMSAdminSite